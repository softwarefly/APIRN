...
object OriginRate {
  ...
  def main(args: Array[String]) {
    ...
    for (month <- 2 to 12) {
      // val month = 2
      // 从HDFS获取初始识别结果集
      val recognitionResult = sc.objectFile[(Long, ((String, Int, Double), (String, Int, Double), (String, Int, Double)))](savePath + "setLabelAddTPIIN&APIRN_V2/month" + month)
      recognitionResult.repartition(60).saveAsObjectFile(savePath + "NewLabelSet2_V2/month" + month)
